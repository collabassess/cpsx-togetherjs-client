...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    requirejs: {
      compile: {
        options: {
          ...
          //Wrap any build2 bundle in a start and end text specified by wrap.
    ...
    watch: {
      ...
      // forget.  Then between git action the build2 will be over-run,
      // but that's harmless.
      minimal: {
  ...
  // For some reason doing ["build2", "buildsite", "watch:site"]
  ...
  grunt.registerTask(
    ...
    function () {
      ...
      var destBase = grunt.option("dest") || "build"; // where to put the built files. If not indicated then into build2/
    ...
    function() {
      ...
      freeport(function(err1, hubPort) {
        freeport(function(err2, webPort) {
          ...
          // build2 togetherjs using these default ports
